    ...
    try:
        ...
        if saved_similarities:
            ...
            for sim in saved_similarities:
                ...
                if paper_details:
                    authors_data = await ml_result_collection.find_one(
                        {"file_id": sim["target_paper_id"]}
                    )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json={"target_paper": target_data, "papers_data": papers_data},
            ) as response:
                if response.status != 200:
                    raise HTTPException(
                        status_code=500, detail="Error computing similarities"
                    )
